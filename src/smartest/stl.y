...
| declaration {
    ArrayList<ASTNode> plist = new ArrayList<ASTNode>();
    plist.add((ASTNode)$1.obj);
    $$ = new ParserVal(plist);
}

statements : statements statement {
    ((StatementsNode)$1.obj).addChild((ASTNode)$2.obj);
    $$ = $1;
}
|/*  empty */ {
...
declaration : type ID {
    $$ = new ParserVal(new DeclarationNode($1.sval, new IDNode($2.sval, true, line, column), line, column));
}

statement : declaration ';' {
    ((DeclarationNode)$1.obj).setIsStatement(true);
    $$ = $1;
}
...
| /* empty */ {
    QuestionListNode ql = new QuestionListNode(line, column);
    $$ = new ParserVal(ql);
}

question_list : question_list ',' expression {
    ((QuestionListNode)$1.obj).addChild((ASTNode)$3.obj);
}
| expression {
    QuestionListNode ql = new QuestionListNode(line, column); ql.addChild((ASTNode)$1.obj);
    $$ = new ParserVal(ql);
}

set_literal : '[' optional_question_list ']' {
    SetLiteralNode sl = new SetLiteralNode((ASTNode)$2.obj, line, column);
    $$ = new ParserVal(sl);
}

set_insert : set_insert INSERT expression {
    ((ASTNode)$1.obj).addChild((ASTNode)$3.obj);
    $$ = $1;
}
...
answer_choices : answer_choices ',' answer_choice {
    ((AnswerChoicesListNode)$1.obj).addAnswer((ASTNode)$3.obj);
    $$ = $1;
}
| answer_choice {
...
| BOOLLITERAL {
    $$ = new ParserVal(new LiteralNode("boolean", (Object) $1.sval,line, column));
}
| ID {
    $$ = new ParserVal(new IDNode($1.sval, false, line, column));
}
| question_literal {
    $$ = $1;
}
...
| '(' expression ')' {
    $$ = new ParserVal(new ParenthesesNode((ASTNode)$2.obj, line, column));
}  	
| function_call	{
    $$ = $1;
}
